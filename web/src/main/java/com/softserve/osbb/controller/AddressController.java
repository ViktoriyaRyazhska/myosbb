...
public class AddressController {
	...
	public AddressDTO addCity(@RequestBody AddressDTO added) {
		City city = addressService
				.addCity(new City(added.getId(), added.getName(), addressService.getRegionById(added.getOwnerId())));
	...
	public AddressDTO updateCity(@RequestBody AddressDTO updated) {
		City city = addressService.updateCity(
				new City(updated.getId(), updated.getName(), addressService.getRegionById(updated.getOwnerId())));
	...
	public AddressDTO addStreet(@RequestBody AddressDTO added) {
		Street street = addressService
				.addStreet(new Street(added.getId(), added.getName(), addressService.getCityById(added.getOwnerId())));
		return new AddressDTO(street.getId(), street.getName(), street.getCity().getId());
	}

	@ResponseStatus(HttpStatus.OK)
	@RequestMapping(value = "/street", method = RequestMethod.PUT)
	public AddressDTO updateStreet(@RequestBody AddressDTO updated) {
		Street street = addressService.updateStreet(
				new Street(updated.getId(), updated.getName(), addressService.getCityById(updated.getOwnerId())));
		return new AddressDTO(street.getId(), street.getName(), null);
	}
	...
	public AddressDTO addDistrict(@RequestBody AddressDTO added) {
		District district = addressService.addDistrict(
				new District(added.getId(), added.getName(), addressService.getCityById(added.getOwnerId())));
	...
	public AddressDTO updateDistrict(@RequestBody AddressDTO updated) {
		District district = addressService.updateDistrict(
				new District(updated.getId(), updated.getName(), addressService.getCityById(updated.getOwnerId())));
		return new AddressDTO(district.getId(), district.getName(), district.getCity().getId());
	}
	...
	public Error illegalArgument(IllegalArgumentException exception) {
		return addressErrorHandler(exception);
	}

	private Error addressErrorHandler(Exception exception) {
